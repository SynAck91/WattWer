class WattWerConfigPanel extends HTMLElement {
      ...
      <div class="page">
        <main class="shell">
          ...
          <nav class="tabs">
            ...
            ${this._tabButton("costs","mdi:cash-multiple","Kosten")}
  ...
  _render() {
    ...
    else if (this._tab === "costs") this._renderCosts();
  ...
  _consumerItem(c) {
    ...
    return `<div class="item ${selected?"selected":""}" data-consumer="${this._attr(c.id)}">
      ...
      <div class="badges"><span class="badge ${c.enabled!==false?"ok":""}">${c.enabled!==false?"Aktiv":"Deaktiviert"}</span>${c.energy_entity_id?'<span class="badge role">kWh-Zähler</span>':''}</div>
    ...
    return `<div class="panehead"><div class="grow"><h2>Verbraucher bearbeiten</h2><p>Name und Mess-Entity des Verbrauchers anpassen.</p></div></div>
      <div class="editor" data-editor-consumer="${this._attr(c.id)}">
        ...
        <div class="section"><div class="sectionTitle"><ha-icon icon="mdi:flash-outline"></ha-icon>Messung</div><div class="formgrid">
          ...
          <label>Energiezähler (optional)</label><div class="entityline"><input id="cEnergyEntity" type="text" value="${this._attr(c.energy_entity_id||"")}" placeholder="sensor.xyz_energy"><button class="secondary" id="browseConsumerEnergy">Durchsuchen</button></div>
          <label>Energiemethode</label><div><select id="cEnergyMode"><option value="auto" ${(c.energy_mode||"auto")==="auto"?"selected":""}>Automatisch (empfohlen)</option><option value="meter_preferred" ${c.energy_mode==="meter_preferred"?"selected":""}>Energiezähler bevorzugen</option><option value="power_only" ${c.energy_mode==="power_only"?"selected":""}>Nur Leistungsintegration</option></select><div class="help">Mit Energiezähler bestimmt dessen kumulative kWh-Differenz den Gesamtverbrauch abgeschlossener 15-Minuten-Fenster. PV/Netz/Batterie werden weiterhin aus den zeitgleichen Leistungswerten bestimmt und proportional auf den Zählerwert normiert. Bei fehlendem, zurückgesetztem oder unplausiblem Zähler greift automatisch die Leistungsintegration.</div></div>
  ...
  _wireConsumerEditor(c) {
    ...
    root.querySelector("#browseConsumerEnergy")?.addEventListener("click",()=>this._openEnergyEntityBrowser(value=>{root.querySelector("#cEnergyEntity").value=value;}));
  ...
  _addConsumerDraft() {
    ...
    this._cfg.consumers.push({id,name:"Neuer Verbraucher",entity_id:"",energy_entity_id:null,energy_mode:"auto",role:"normal",enabled:true,icon:"mdi:flash",description:""});
  ...
  _readCurrentConsumer() {
    ...
    return {...old,
      ...
      energy_entity_id:root.querySelector("#cEnergyEntity")?.value.trim()||null,
      energy_mode:root.querySelector("#cEnergyMode")?.value||"auto",
  ...
  _renderGenerators() {
    ...
    const visible=all.filter(g=>!filter || `${g.name} ${g.entity_id} ${g.fallback_entity_id||""} ${g.energy_entity_id||""}`.toLowerCase().includes(filter));
  ...
  _generatorItem(g){
    ...
    return `<div class="item ${selected?"selected":""}" data-generator="${this._attr(g.id)}">
      ...
      <div class="badges"><span class="badge ${g.enabled!==false?"ok":""}">${g.enabled!==false?"Aktiv":"Deaktiviert"}</span><span class="badge role">${this._esc(target)}</span>${g.energy_entity_id?'<span class="badge role">kWh-Zähler</span>':''}</div>
      <ha-icon icon="mdi:chevron-right"></ha-icon>
    </div>`;
  }
    ...
    return `<div class="panehead"><div class="grow"><h2>PV-Erzeuger bearbeiten</h2><p>Messquelle, Fallback und elektrische Zuordnung festlegen.</p></div></div>
      <div class="editor" data-editor-generator="${this._attr(g.id)}">
        ...
        <div class="section"><div class="sectionTitle"><ha-icon icon="mdi:solar-panel"></ha-icon>Messung</div><div class="formgrid">
          ...
          <label>Erzeugungs-Vorzeichen</label><select id="gPolarity"><option value="positive" ${(g.polarity||"positive")==="positive"?"selected":""}>Positive Werte = Erzeugung (+W)</option><option value="negative" ${g.polarity==="negative"?"selected":""}>Negative Werte = Erzeugung (−W)</option></select>
          ...
          <label>Fallback-Vorzeichen</label><select id="gFallbackPolarity"><option value="same" ${(g.fallback_polarity||"same")==="same"?"selected":""}>Wie Hauptsensor</option><option value="positive" ${g.fallback_polarity==="positive"?"selected":""}>Positive Werte = Erzeugung (+W)</option><option value="negative" ${g.fallback_polarity==="negative"?"selected":""}>Negative Werte = Erzeugung (−W)</option></select>
          <label>PV-Energiezähler (optional)</label><div class="entityline"><input id="gEnergyEntity" type="text" value="${this._attr(g.energy_entity_id||"")}" placeholder="sensor.pv_energy"><button class="secondary" id="browseGeneratorEnergy">Durchsuchen</button></div>
          <label>Energiemethode</label><div><select id="gEnergyMode"><option value="auto" ${(g.energy_mode||"auto")==="auto"?"selected":""}>Automatisch (empfohlen)</option><option value="meter_preferred" ${g.energy_mode==="meter_preferred"?"selected":""}>Energiezähler bevorzugen</option><option value="power_only" ${g.energy_mode==="power_only"?"selected":""}>Nur Leistungsintegration</option></select><div class="help">Der kumulative PV-kWh-Zähler kalibriert die erzeugte Energie des Erzeugers pro abgeschlossenem 15-Minuten-Fenster und wird auch beim Backfill ausgewertet. Die Verbraucher-PV-Anteile werden nicht blind auf diesen Zähler skaliert, weil erzeugte Energie auch eingespeist oder in eine Batterie geladen werden kann.</div></div>
        ...
        </div><div class="help"><b>Vorzeichen:</b> Stelle ein, ob dein Messgerät Erzeugung als positiven oder negativen Wert liefert. Der Fallback kann eine andere Konvention haben (z. B. Shelly −W, DTU +W). WattWer wandelt intern immer auf positive Erzeugungsleistung um. Der Fallback-Sensor wird verwendet, wenn er frischer als die primäre Messung ist. Ein Nacht-Nullwert wird nur angewendet, wenn Home Assistant <code>sun.sun = below_horizon</code> kennt.</div></div>
        ...
        <div class="section"><div class="sectionTitle"><ha-icon icon="mdi:currency-eur"></ha-icon>PV-Kostenpreis</div><p class="help">Preisverlauf dieses Erzeugers pro kWh. Neue Preise werden mit einem Gültigkeitsdatum ergänzt; ältere Einträge bleiben für historische Kosten erhalten.</p><div id="generatorTariffs">${this._tariffRowsHtml(g.tariffs||[],"g")}</div><button class="secondary" type="button" id="addGeneratorTariff">+ Preisänderung</button></div>
  ...
  _wireGeneratorEditor(g){
    ...
    root.querySelector("#browseGeneratorEnergy")?.addEventListener("click",()=>this._openEnergyEntityBrowser(v=>{root.querySelector("#gEnergyEntity").value=v;}));
    ...
    this._wireTariffRows(root.querySelector("#generatorTariffs"),"g");
    root.querySelector("#addGeneratorTariff")?.addEventListener("click",()=>this._addTariffRow(root.querySelector("#generatorTariffs"),"g"));
  ...
  _addGeneratorDraft(){
    ...
    this._cfg.generators.push({id,name:"Neuer PV-Erzeuger",entity_id:"",fallback_entity_id:"",polarity:"positive",fallback_polarity:"same",energy_entity_id:null,energy_mode:"auto",role:"main_bus",consumer_id:null,enabled:true,night_zero:true,max_age:180,icon:"mdi:solar-power",description:"",tariffs:[]});
  ...
  _renderGeneral(){const s=this._cfg.settings||{};const body=this.shadowRoot.getElementById("body");body.innerHTML=`<div class="settings">
    ...
    <div class="settingCard"><h3>Messwert-Synchronisierung</h3><p>Richtet asynchron gemeldete Netz-, Verbraucher- und PV-Werte auf einen gemeinsamen Zielzeitpunkt aus. WattWer lernt zusätzlich für jeden Sensor dessen typisches Meldeintervall.</p>
      <div class="field"><label>Synchronisierung</label><div class="switchline"><label class="toggle"><input id="set_sync_enabled" type="checkbox" ${s.sync_enabled!==false?"checked":""}><span class="slider"></span></label><span>aktiv</span></div></div>
      <div class="field"><label>Adaptive Sensor-Frische</label><div class="switchline"><label class="toggle"><input id="set_adaptive_freshness" type="checkbox" ${s.adaptive_freshness!==false?"checked":""}><span class="slider"></span></label><span>pro Sensor lernen</span></div><div class="help">Empfohlen: aktiv. WattWer lernt Median und Jitter der Meldeintervalle jedes Netz-, Verbraucher-, Batterie- und PV-Sensors. Ein Sensor wird zunächst nur als verzögert markiert und erst am harten Timeout verworfen.</div></div>
      <div class="field"><label>Harter Timeout normale Sensoren (s)</label><input type="number" min="15" max="600" step="1" id="set_adaptive_hard_timeout" value="${this._attr(s.adaptive_hard_timeout??60)}"><div class="help">Empfohlen: 60 s. Spätestens danach gilt ein normaler Sensor als veraltet. PV-Erzeuger verwenden ihren eigenen konfigurierten Max.-Sensoralter-Wert.</div></div>
      <div class="field"><label>Berechnungsverzögerung (s)</label><input type="number" min="0" max="30" step="0.5" id="set_sync_delay" value="${this._attr(s.sync_delay??5)}"><div class="help">Empfohlen: 5 s. Die Verzögerung gibt asynchronen Integrationen Zeit, ihre Messwerte zu melden.</div></div>
      <div class="field"><label>Messwertpuffer (s)</label><input type="number" min="10" max="300" step="1" id="set_sync_buffer" value="${this._attr(s.sync_buffer??30)}"><div class="help">Empfohlen: 30 s. WattWer erweitert den internen Puffer automatisch, wenn ein gelernter/harter Timeout mehr Historie benötigt.</div></div>
      <div class="field"><label>Feste Max.-Sample-Zeit bei deaktivierter Lernlogik (s)</label><input type="number" min="2" max="120" step="1" id="set_sync_max_sample_age" value="${this._attr(s.sync_max_sample_age??10)}"><div class="help">Wird nur verwendet, wenn „Adaptive Sensor-Frische“ ausgeschaltet ist.</div></div>
    </div>
  ...
  async _saveSettings(){if(this._busy)return;const b=this.shadowRoot.getElementById("body");const s={...(this._cfg.settings||{})};["grid_import","grid_export","house_net","battery_charge","battery_discharge"].forEach(k=>s[k]=b.querySelector(`#set_${k}`)?.value.trim()||"");["sample_interval","max_age","deadband","quarter_retention_days","hour_retention_days","sync_delay","sync_buffer","sync_max_sample_age","adaptive_hard_timeout"].forEach(k=>s[k]=Number(b.querySelector(`#set_${k}`)?.value));s.sync_enabled=!!b.querySelector("#set_sync_enabled")?.checked;s.adaptive_freshness=!!b.querySelector("#set_adaptive_freshness")?.checked;s.background_loads=(b.querySelector("#set_background_loads")?.value||"").split(/[\n,;]+/).map(x=>x.trim()).filter(Boolean);this._busy=true;try{await this._hass.callWS({type:"pv_energy_allocation/config/settings_update",settings:s});this._message("Einstellungen gespeichert. WattWer wird neu geladen …",true);await this._waitReload();await this._load();}catch(err){this._message(`Speichern fehlgeschlagen: ${String(err?.message||err)}`,false);}finally{this._busy=false;}}

  _tariffRowsHtml(tariffs,prefix){const rows=(tariffs||[]).length?tariffs:[{valid_from:this._date(new Date()),price_per_kwh:0}];return rows.map((t,i)=>`<div class="entityline tariffrow" style="margin-bottom:8px"><input class="${prefix}TariffDate" type="date" value="${this._attr(t.valid_from||"")}"><input class="${prefix}TariffPrice" type="number" min="0" step="0.0001" value="${this._attr(t.price_per_kwh??0)}" placeholder="Preis / kWh"><button class="danger removeTariff" type="button">Entfernen</button></div>`).join("");}
  _wireTariffRows(host,prefix){host?.querySelectorAll(".removeTariff").forEach(b=>b.onclick=()=>b.closest(".tariffrow")?.remove());}
  _addTariffRow(host,prefix){if(!host)return;const wrap=document.createElement("div");wrap.innerHTML=this._tariffRowsHtml([{valid_from:this._date(new Date()),price_per_kwh:0}],prefix);host.appendChild(wrap.firstElementChild);this._wireTariffRows(host,prefix);}
  _collectTariffs(host,prefix){if(!host)return[];const rows=[...host.querySelectorAll(".tariffrow")].map(r=>({valid_from:r.querySelector(`.${prefix}TariffDate`)?.value||"",price_per_kwh:Number(r.querySelector(`.${prefix}TariffPrice`)?.value)})).filter(x=>x.valid_from&&Number.isFinite(x.price_per_kwh)&&x.price_per_kwh>=0);rows.sort((a,b)=>a.valid_from.localeCompare(b.valid_from));return rows;}

  _renderCosts(){const s=this._cfg.settings||{},body=this.shadowRoot.getElementById("body");body.innerHTML=`<div class="settings"><div class="settingCard"><h3>Abrechnung</h3><p>WattWer verwendet Preisverläufe mit Gültigkeitsdatum. Eine Preisänderung ab 01.01.2027 verändert daher keine Kosten aus 2026.</p><div class="field"><label>Währung</label><input id="costCurrency" value="${this._attr(s.currency||"EUR")}" maxlength="8"></div><h4>Netzbezug</h4><div id="gridTariffs">${this._tariffRowsHtml(s.grid_tariffs||[],"grid")}</div><button class="secondary" type="button" id="addGridTariff">+ Preisänderung</button>${(s.battery_charge&&s.battery_discharge)?`<h4 style="margin-top:20px">Batterieentladung</h4><div class="help">Optionaler Kostenpreis der Batterieentladung, z. B. inklusive Speicherverlusten/Abschreibung. Ohne Preis bleiben Batterie-kWh in der Kostenabdeckung offen.</div><div id="batteryTariffs">${this._tariffRowsHtml(s.battery_tariffs||[],"bat")}</div><button class="secondary" type="button" id="addBatteryTariff">+ Preisänderung</button>`:""}<div class="actions" style="margin-top:18px"><button class="primary" id="saveCosts">Kosten speichern</button></div></div><div class="settingCard"><h3>PV-Erzeuger</h3><p>PV-Preise werden je Erzeuger direkt im Tab „PV-Erzeugung“ gepflegt. Dadurch kann z. B. Dach Süd einen anderen kWh-Kostenpreis als ein Balkonkraftwerk haben.</p><div class="help">Die Kostenformel lautet: Netz-kWh × Netzpreis + Summe(PV-kWh je Erzeuger × dessen Preis) + Batterie-kWh × Batteriepreis. Nicht eindeutig einem PV-Erzeuger zuordenbare kWh werden als ungepreist gekennzeichnet statt geschätzt.</div></div></div>`;this._wireTariffRows(body.querySelector("#gridTariffs"),"grid");this._wireTariffRows(body.querySelector("#batteryTariffs"),"bat");body.querySelector("#addGridTariff")?.addEventListener("click",()=>this._addTariffRow(body.querySelector("#gridTariffs"),"grid"));body.querySelector("#addBatteryTariff")?.addEventListener("click",()=>this._addTariffRow(body.querySelector("#batteryTariffs"),"bat"));body.querySelector("#saveCosts")?.addEventListener("click",()=>this._saveCosts());}
  async _saveCosts(){if(this._busy)return;const b=this.shadowRoot.getElementById("body"),base={...(this._cfg.settings||{})};base.grid_tariffs=this._collectTariffs(b.querySelector("#gridTariffs"),"grid");base.battery_tariffs=this._collectTariffs(b.querySelector("#batteryTariffs"),"bat");base.currency=(b.querySelector("#costCurrency")?.value||"EUR").trim().toUpperCase();this._busy=true;try{await this._hass.callWS({type:"pv_energy_allocation/config/settings_update",settings:base});this._message("Kostenverlauf gespeichert. WattWer wird neu geladen …",true);await this._waitReload();await this._load();}catch(err){this._message(`Speichern fehlgeschlagen: ${String(err?.message||err)}`,false);}finally{this._busy=false;}}
  ...
  _openEnergyEntityBrowser(onPick){const candidates=Object.values(this._hass.states||{}).filter(s=>s.entity_id?.startsWith("sensor.")).filter(s=>{const dc=String(s.attributes?.device_class||"").toLowerCase(),sc=String(s.attributes?.state_class||"").toLowerCase(),u=String(s.attributes?.unit_of_measurement||"").toLowerCase();return dc==="energy"||["wh","kwh","mwh","j","kj","mj"].includes(u);}).sort((a,b)=>(a.attributes?.friendly_name||a.entity_id).localeCompare(b.attributes?.friendly_name||b.entity_id));const host=this.shadowRoot.getElementById("modalHost");host.innerHTML=`<div class="modalback"><div class="modal"><div class="modalhead"><h3>Energiezähler auswählen</h3><button class="iconbtn" id="energyClose"><ha-icon icon="mdi:close"></ha-icon></button></div><div class="modalbody"><div class="search" style="margin-bottom:9px"><ha-icon icon="mdi:magnify"></ha-icon><input id="energySearch" placeholder="Entity oder Name suchen…"></div><div id="energyList"></div></div></div></div>`;const render=()=>{const q=host.querySelector("#energySearch").value.toLowerCase();const rows=candidates.filter(s=>`${s.entity_id} ${s.attributes?.friendly_name||""}`.toLowerCase().includes(q));host.querySelector("#energyList").innerHTML=rows.map(s=>`<div class="entityItem" data-pick-energy="${this._attr(s.entity_id)}"><strong>${this._esc(s.attributes?.friendly_name||s.entity_id)}</strong><span>${this._esc(s.entity_id)} · ${this._esc(s.state)} ${this._esc(s.attributes?.unit_of_measurement||"")}</span></div>`).join("")||'<div class="empty">Keine passenden Energiezähler gefunden.</div>';host.querySelectorAll("[data-pick-energy]").forEach(x=>x.onclick=()=>{onPick(x.dataset.pickEnergy);host.innerHTML="";});};host.querySelector("#energyClose").onclick=()=>host.innerHTML="";host.querySelector("#energySearch").oninput=render;render();host.querySelector("#energySearch").focus();}
