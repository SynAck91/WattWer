class WattWerConfigPanel extends HTMLElement {
  ...
  _renderGeneral(){const s=this._cfg.settings||{};const body=this.shadowRoot.getElementById("body");body.innerHTML=`<div class="settings">
    ...
    <div class="settingCard"><h3>Messwert-Synchronisierung</h3><p>Richtet asynchron gemeldete Netz-, Verbraucher- und Erzeugungswerte auf einen gemeinsamen Zielzeitpunkt aus. WattWer verwendet dabei immer den letzten gemeldeten Wert vor oder genau am Zielzeitpunkt (Sample-and-Hold).</p>
      ...
      <div class="field"><label>Berechnungsverzögerung (s)</label><input type="number" min="0" max="30" step="0.5" id="set_sync_delay" value="${this._attr(s.sync_delay??5)}"><div class="help">Empfohlen: 5 s. Die zusätzliche Verzögerung gibt langsameren Sensoren Zeit, ihren Messwert zu melden.</div></div>
      <div class="field"><label>Messwertpuffer (s)</label><input type="number" min="10" max="300" step="1" id="set_sync_buffer" value="${this._attr(s.sync_buffer??30)}"><div class="help">Empfohlen: 30 s. Muss mindestens Verzögerung + maximales Sample-Alter abdecken.</div></div>
      <div class="field"><label>Max. Sample-Alter für normale Quellen (s)</label><input type="number" min="2" max="120" step="1" id="set_sync_max_sample_age" value="${this._attr(s.sync_max_sample_age??10)}"><div class="help">Empfohlen: 10 s. PV-Erzeuger können weiterhin ihr eigenes größeres Sensoralter verwenden.</div></div>
    </div>
